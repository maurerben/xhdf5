...
contains
   ...
   subroutine check_hyperslabs(this, calling_routine, mpi_comm)
      ...
      call assert_true(mpi_comm, size(this) > 0, &
         'Error(solhdf5: '// calling_routine //'): hyperslab array is empty.')

      do hsdx=1, size(this)

         call assert_true(mpi_comm, this(hsdx)%rank /= uninitialized_rank, &
            'Error(solhdf5: '// calling_routine //'): hyperslab is not initialized.')

         call assert_true(mpi_comm, allocated(this(hsdx)%count), &
            'Error(solhdf5: '// calling_routine //'): hyperslab count is not allocated.')
         call assert_true(mpi_comm, allocated(this(hsdx)%datasize), &
            'Error(solhdf5: '// calling_routine //'): hyperslab datasize is not allocated.')
         call assert_true(mpi_comm, allocated(this(hsdx)%memsize), &
            'Error(solhdf5: '// calling_routine //'): hyperslab memsize is not allocated.')
         call assert_true(mpi_comm, allocated(this(hsdx)%datastart), &
            'Error(solhdf5: '// calling_routine //'): hyperslab datastart is not allocated.')
         call assert_true(mpi_comm, allocated(this(hsdx)%memstart), &
            'Error(solhdf5: '// calling_routine //'): hyperslab memstart is not allocated.')
         call assert_true(mpi_comm, allocated(this(hsdx)%stride), &
            'Error(solhdf5: '// calling_routine //'): hyperslab stride is not allocated.')
         call assert_true(mpi_comm, allocated(this(hsdx)%datablock), &
            'Error(solhdf5: '// calling_routine //'): hyperslab datablock is not allocated.')

         call assert_true(mpi_comm, size(this(hsdx)%count) == this(hsdx)%rank, &
            'Error(solhdf5: '// calling_routine //'): size(count) /= rank.')
         call assert_true(mpi_comm, size(this(hsdx)%datasize) == this(hsdx)%rank, &
            'Error(solhdf5: '// calling_routine //'): size(datasize) /= rank.')
         call assert_true(mpi_comm, size(this(hsdx)%memsize) == this(hsdx)%rank, &
            'Error(solhdf5: '// calling_routine //'): size(memsize) /= rank.')
         call assert_true(mpi_comm, size(this(hsdx)%datastart) == this(hsdx)%rank, &
            'Error(solhdf5: '// calling_routine //'): size(datastart) /= rank.')
         call assert_true(mpi_comm, size(this(hsdx)%memstart) == this(hsdx)%rank, &
            'Error(solhdf5: '// calling_routine //'): size(memstart) /= rank.')
         call assert_true(mpi_comm, size(this(hsdx)%stride) == this(hsdx)%rank, &
            'Error(solhdf5: '// calling_routine //'): size(stride) /= rank.')
         call assert_true(mpi_comm, size(this(hsdx)%datablock) == this(hsdx)%rank, &
            'Error(solhdf5: '// calling_routine //'): size(datablock) /= rank.')

         call assert_true(mpi_comm, this(hsdx)%rank == this(1)%rank, &
            'Error(solhdf5: '// calling_routine //'): hyperslabs have inconsistent rank.')
         call assert_true(mpi_comm, all(this(hsdx)%datasize == this(1)%datasize), &
            'Error(solhdf5: '// calling_routine //'): hyperslabs have inconsistent datasize.')
         call assert_true(mpi_comm, all(this(hsdx)%memsize == this(1)%memsize), &
            'Error(solhdf5: '// calling_routine //'): hyperslabs have inconsistent memsize.')

         call assert_true(mpi_comm, all(this(hsdx)%datasize > 0), &
            'Error(solhdf5: '// calling_routine //'): Some elements of datasize <= 0.')
         call assert_true(mpi_comm, all(this(hsdx)%memsize >= 0), &
            'Error(solhdf5: '// calling_routine //'): Some elements of memsize < 0.')
         call assert_true(mpi_comm, all(this(hsdx)%count >= 0), &
            'Error(solhdf5: '// calling_routine //'): Some elements of count < 0.')
         call assert_true(mpi_comm, all(this(hsdx)%stride > 0), &
            'Error(solhdf5: '// calling_routine //'): Some elements of stride <= 0.')
         call assert_true(mpi_comm, all(this(hsdx)%datablock > 0), &
            'Error(solhdf5: '// calling_routine //'): Some elements of datablock <= 0.')
         call assert_true(mpi_comm, all(this(hsdx)%datastart >= 0), &
            'Error(solhdf5: '// calling_routine //'): Some elements of datastart < 0.')
         call assert_true(mpi_comm, all(this(hsdx)%memstart >= 0), &
         ...
         if(all(this(hsdx)%count > 0)) then
            ...
            call assert_true(mpi_comm, all(file_last_exclusive <= this(hsdx)%datasize), &
               'Error(solhdf5: '// calling_routine //'): hyperslab exceeds datasize.')
            call assert_true(mpi_comm, all(mem_last_exclusive <= this(hsdx)%memsize), &
